...
* Xaringan Slides "on steroids" (i.e. `{xaringanExtra}`) 
* This simple plot:
...
-   More complex than `{automagic}`
  - More steps involved
  - `renv.lock` is more verbose than `deps.yml`
...
* Packages (and versions!) need to be stored and shared
  * Avoid not being able to reproduce something because of missing packages
...
* All these options will recreate .highlight[R packages only]
